...
Current release: **`packaging/wifi-fallback_0.6.7_arm64.deb`** (checksum in `.sha256` alongside it).

Copy the deb and `install.sh` to the reader, then:

```bash
sudo ./install.sh wifi-fallback_0.6.7_arm64.deb
```
...
```bash
sudo apt install ./wifi-fallback_0.6.7_arm64.deb   # note the ./ — apt resolves deps
sudo dpkg -i wifi-fallback_0.6.7_arm64.deb && sudo apt -f -y install
```
...
```bash
cd packaging && sha256sum wifi-fallback_0.6.7_arm64.deb > wifi-fallback_0.6.7_arm64.deb.sha256
```
